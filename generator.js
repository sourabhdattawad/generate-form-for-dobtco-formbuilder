function dynamicUIGenerator(fieldString) {
    var output = "";
    var field = JSON.parse(fieldString).fields;
    for (key in field) {
        console.log("inside for loop" + key);
        console.log(field[key].field_type);
        output += "<div class='form-group'>";
        output += "<label class='col-md-4 control-label' >" + field[key].label + "</label> ";
        ...
        switch (field[key].field_type) {
            ...
            case "email":
                ...
                break;
            case "paragraph":
                var paraGraphField = '<textarea class="form-control" rows="3"  name =' + '"' + field[key].label + '"' + ' id=' + '"' + field[key].label + '"' + ' required="">' + '</textarea>'
            ...
            case "radio":
                ...
                break;
            case "price":
                output += '<input type= "number"' + ' name = ' + '"' + field[key].label + '"' + ' id=' + '"' + field[key].label + '"' + ' value="" required=""/>'
                break;
            case "address":
                var address = '';
